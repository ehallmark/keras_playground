        ...
        sql = '''
            insert into {{N}} (
                select
                    ...
                    max(coalesce(p2.round_num,0))/7.0,
                    case when bool_and(p2.player_victory) then 0 else 1 end as losses,
                    ...
                    sum(coalesce(p2.masters,0))/(greatest(1,2000.0*count(p2.*))) as avg_tournament_rank,
                    coalesce(sum(case when p2.player_victory then p2.masters else 0 end), 0)/3000.0 as atp_points,
                    sum(p2.games_won-p2.games_against)::double precision/(12.0*greatest(1,count(p2.*))) as spread_avg,
                    coalesce(sum(case when p2.court_surface='Grass' then 1.0 else 0.0 end)/count(p2.*), 0) as grass_percent,
                    coalesce(sum(case when p2.court_surface='Clay' then 1.0 else 0.0 end)/count(p2.*), 0) as clay_percent,
                    coalesce(sum(case when p2.court_surface='Hard' then 1.0 else 0.0 end)/count(p2.*), 0) as hard_percent,
                    coalesce(sum(case when player1.country is null then 0.0 else case when player1.country = coalesce(country.code, p2.nation) then 1.0 else 0.0 end end)/count(p2.*), 0.0) as local_percent,
