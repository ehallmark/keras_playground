...
class TableCreator:
    ...
    def attributes_for(self, i, include_opp=True, opp_only=False):
        attrs = [
            ...
            self.prefix + str(i) + '.' + 'hard_percent as hard_percent_' + self.prefix + str(i),
    ...
    def attribute_names_for(self, i, include_opp=True, opp_only=False):
        attrs = [
            ...
            'hard_percent_' + self.prefix + str(i),
            'local_percent_' + self.prefix + str(i),
            'qualifier_percent_' + self.prefix + str(i),
        ]
        if include_opp or opp_only:
        ...
        sql = '''
            insert into {{N}} (
                select distinct on (player_id,start_date,tournament) * from (
                    ...
                    sum(case when m2.player_victory then 1 else 0 end)/greatest(1,count(m2.*)) as victories,
                    ...
                    sum(coalesce(m2.masters,0))/(greatest(1,2000.0*count(m2.*))) as avg_tournament_rank,
                    coalesce(sum(case when m2.player_victory then m2.masters else 0 end), 0)/3000.0 as atp_points,
                    sum(case when t.masters < m2.masters then 1.0 else 0.0 end)/greatest(1, count(m2.*)) as better_encounters,
                    sum(case when t.masters > m2.masters then 1.0 else 0.0 end)/greatest(1, count(m2.*)) as worse_encounters,
                    sum(m2.games_won-m2.games_against)::double precision/(12.0*greatest(1,count(m2.*))) as spread_avg,
                    coalesce(sum(case when m2.court_surface='Grass' then 1.0 else 0.0 end)/count(m2.*), 0) as grass_percent,
                    coalesce(sum(case when m2.court_surface='Clay' then 1.0 else 0.0 end)/count(m2.*), 0) as clay_percent,
                    coalesce(sum(case when m2.court_surface='Hard' then 1.0 else 0.0 end)/count(m2.*), 0) as hard_percent,
                    coalesce(sum(case when player1.country is null then 0.0 else case when player1.country = coalesce(country.code, m2.nation) then 1.0 else 0.0 end end)/count(m2.*), 0.0) as local_percent,
                    ...
                    join atp_matches_individual_with_meta as m2
                    on (
...
#                              join_table_name='atp_matches_juniors_all', time_period=60, where_str="m2.masters = 0")

#itf_tables = TableCreator(prefix='itf', table='atp_matches_itf', num_tables=1,
#                              join_table_name='atp_matches_itf_all', time_period=60, where_str="m2.masters = 25")

challenger_tables = TableCreator(prefix='ch', table='atp_matches_challenger', num_tables=1,
                              join_table_name='atp_matches_challenger_all', time_period=60, where_str="m2.masters = 100")

pro_tables = TableCreator(prefix='pro', table='atp_matches_pro', num_tables=1,
                              join_table_name='atp_matches_pro_all', time_period=60, where_str="m2.masters > 200")

if __name__ == '__main__':
    threads = []
    for func in [challenger_tables.build_tables, pro_tables.build_tables]:
