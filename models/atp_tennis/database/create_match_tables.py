...
def load_data(date='2000-01-01', include_null=False):
    conn = create_engine("postgresql://localhost/ib_db?user=postgres&password=password")
    sql = 'select case when m.player_victory then 1.0 else 0.0 end as y, m.start_date as start_date,m.tournament as tournament,m.player_id as player_id,m.opponent_id as opponent_id,' + ','.join(all_attributes) + ' from atp_matches_individual as m ' + ' '.join([join_str(i) for i in range(num_prior_quarters)])  \
